...
//Use NewCreateProcessA for Global Hooking
BOOL WINAPI NewCreateProcessA(
    ...
    //unhook_by_code("kernel32.dll", "CreateProcessW", (LPVOID)((DWORD)&NewCreateProcessW), OrgFP);
    FARPROC pFunc = (FARPROC)GetProcAddress(GetModuleHandleA("kernel32.dll"), "CreateProcessA");
    pFunc = (FARPROC)((DWORD)pFunc + 2);
    BOOL ret = ((PFCREATEPROCESSA)pFunc)(lpApplicationName,
        lpCommandLine,
        lpProcessAttributes,
        lpThreadAttributes,
        bInheritHandles,
    ...
    if (ret)
        InjectDll(lpProcessInformation->hProcess, DLLNAME);
    DebugLog("DllMain() : NewCreateProcessA\n");
    return ret;
}